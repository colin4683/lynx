...
export const load = async ({params, url, depends}) => {
    ...
    metrics = await db.execute(sql`
        WITH time_slots AS (SELECT to_char(
           ...
           , component_stats AS (
        SELECT DISTINCT
        ON (time_slot, component->>'label')
            time_slot,
            component->>'label' as component_name,
            AVG ((component->>'temperature'):: numeric) as avg_temp
        FROM time_slots, jsonb_array_elements(components::jsonb) as component
        ...
        SELECT t.time_slot,
               ...
               (SELECT jsonb_agg(jsonb_build_object(
                ...
                WHERE cs.time_slot = t.time_slot) as components
        FROM time_slots t
    ...
    if (intervalMinutes === 1) {
        disks = await db.execute(sql`
            SELECT mount_point,
                   ...
                   used                                   as used_total, space as total, read as read_total, write as write_total
            FROM disks
            WHERE
                system = ${path_number}
              AND mount_point = '/'
