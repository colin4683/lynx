<script lang="ts">
	...
	let notifiers = $derived.by(() => data.notifiers ?? []);
...
<div class="">
	<div class="mt-4 flex w-full flex-col items-start gap-2">
		<div class="fixed right-2/3 mb-4 flex items-center justify-between px-20 align-middle">
			<button
				class="bg-background border-border flex items-center rounded border px-3 py-1 align-middle hover:bg-[var(--foreground)]"
				onclick={() => window.history.back()}
			>
				<span class="icon-[line-md--arrow-left] h-5 w-5"></span>
				Back
			</button>
		</div>
		<h1 class="text-2xl font-bold">Create Rule</h1>
		<p class="text-muted-foreground text-sm">Build expressions to create complex alert rules.</p>
		<Switch checked={true} />
		...
		<div class="w-1/2 space-y-2">
			{#each notifiers as notifier}
				<div class="flex w-full items-center justify-between gap-2">
					...
					<Switch id={'notifier-' + notifier.id} class="cursor-pointer" onCheckedChange={() => toggleNotifier(notifier.id)} />
				</div>
			{/each}
		</div>
	</div>
